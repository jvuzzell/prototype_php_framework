...
class Environment_configuration {
    ...
    Public function __construct( array $args ) {
        ...
        $this->site_directory = $args[ 'site_directory' ];
        $this->key = $args[ 'encryption_key' ];
        $this->nonce = $args[ 'encryption_nonce' ];

        $this->directory_search = $args[ 'dependencies' ][ 'classes' ][ 'directory_search_class' ]; 
        $this->get_response = $args[ 'dependencies' ][ 'methods' ][ 'get_response' ];
        $this->encrypt = $args[ 'dependencies' ][ 'methods' ][ 'encryption_method' ]; 
        $this->decrypt = $args[ 'dependencies' ][ 'methods' ][ 'decryption_method' ];

        // call_user_func( $this->encrypt, 'thing', $this->nonce, $this->key );
        $is_cli = $this->is_cli();    
        ...
        $program_directories_response = $this->get_program_directories( realpath( $this->get_env_var_path . '/../' ), $domain_name );
        ...
        $this->env_config = array(
            ...
            'app_path' => $app_path,
            'directories' => array(
                'portfolio' => $program_directories[ 'portfolio' ], 
                'file_storage' => $program_directories[ 'file_storage' ], 
                'tests' => $program_directories[ 'tests' ], 
                'static_assets' => $program_directories[ 'static_assets' ]
            )
        );
    ...
    Public function get_client( string $client_name ) { 
        ...
        if( isset( $this->clients[ $client_name ] ) ) {
            ...
            $response = call_user_func( $this->get_response, array(
                'error'    => false,
                'status'   => 200,
        ...
        if( 
            ...
            foreach( $matching_domain_directories as $directory => $relative_path ) {
                ...
                switch( $app ) {
                    ...
                    case 'tests' : 
                        $program_directories[ 'tests' ][ 'site_specific' ] = $directory . '/' . $domain_dir;
                        $program_directories[ 'tests' ][ 'shared' ] = $directory . '/'; 
                        break;
